...
For **local** `wrangler dev`, put the same four names in a `.dev.vars`
file (gitignored, never committed) instead of running `wrangler secret
put` — see `apps/web/wrangler.toml`'s header comment.
...
Run this as an explicit deploy step, before or after `wrangler deploy` —
it is not run automatically on startup (same posture as the Node profile,
which runs `pnpm --filter @bandlib/db migrate` as its own step, not on
boot).
...
wrangler d1 migrations apply DB --remote           # on a real account
...
wrangler d1 migrations apply DB --local
# .dev.vars with the four secrets above (fake values are fine locally —
# the app only needs them to be present and well-formed)
wrangler dev
```
...
real (though offline-by-default) local Workers runtime. Object storage
and outbound mail calls will fail against fake local credentials (no
MinIO/real provider reachable), but every route that doesn't touch those
two integrations works end-to-end, including the full sign-in flow.
