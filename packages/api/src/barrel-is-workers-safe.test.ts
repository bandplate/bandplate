...
// `@bandlib/api`'s barrel (`index.ts`) never pulls a `node:*` built-in
...
//
// Like `@bandlib/core`, `@bandlib/api` has no existing Node-only sibling
...
// devDependencies on `@bandlib/mail`/`@bandlib/storage` are test-only,
...
describe("@bandlib/api barrel is Workers-safe", () => {
