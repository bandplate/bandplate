...
// `@bandplate/db`'s main barrel (`index.ts`) never pulls a `node:*`
// built-in into a Workers-targeted bundle. Mirrors `@bandplate/mail`'s and
...
//
// `@bandplate/db`'s whole increment-7 premise (`createD1Db` — see
// `client.ts`) rests on this package running unmodified on Workers; the
// closest thing it has to `@bandplate/mail`'s `smtp.ts`/`@bandplate/storage`'s
...
describe("@bandplate/db barrel is Workers-safe", () => {
