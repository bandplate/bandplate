...
// `BANDPLATE_DATABASE_URL`/`DATABASE_URL` and threw on neither, while the
...
export interface DatabaseUrlEnv {
  BANDPLATE_DATABASE_URL?: string | undefined;
  DATABASE_URL?: string | undefined;
}

/**
 * `BANDPLATE_DATABASE_URL` is the name `apps/web` itself requires; the bare
...
export function resolveDatabaseUrl(
  ...
  const url = env.BANDPLATE_DATABASE_URL || env.DATABASE_URL;
  if (!url) {
    throw new Error(`Set BANDPLATE_DATABASE_URL (or DATABASE_URL) before ${action}.`);
  }
  return url;
}
